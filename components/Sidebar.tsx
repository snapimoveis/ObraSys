...
import { 
  LayoutDashboard, Calculator, HardHat, Calendar, Users, 
  FileCheck, Wallet, FileText, Settings, Database, Briefcase, 
  Zap, LogOut, Bolt, LifeBuoy 
} from 'lucide-react';
...
const Sidebar: React.FC<SidebarProps> = ({ currentView, setCurrentView, isMobileOpen, setIsMobileOpen }) => {
  const { userName, logout, role } = useSession();
  ...
  return (
    <>
      ...
      <aside className={`fixed top-0 left-0 z-30 h-screen w-64 bg-white border-r border-slate-200 text-slate-600 transition-transform duration-300 ease-in-out flex flex-col ${isMobileOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 lg:static`}>
        ...
        <div className="p-6 pb-2 border-b border-slate-100 flex items-center gap-3">
          ...
          <div className="overflow-hidden">
            <p className="text-sm font-bold text-slate-800 truncate">{userName || 'Utilizador'}</p>
            <p className="text-[10px] text-slate-400 mt-0.5 font-bold uppercase tracking-wider">{role}</p>
          </div>
        </div>
        ...
        <nav className="flex-1 overflow-y-auto py-4 px-3 space-y-1 custom-scrollbar">
          {menuItems.map((item) => {
            ...
            return (
              <button key={item.id} onClick={() => { setCurrentView(item.id); setIsMobileOpen(false); }} className={`w-full flex items-center space-x-3 px-3 py-2.5 rounded-lg transition-colors text-sm font-medium ${isActive ? 'bg-[#34669A] text-white shadow-sm' : 'text-slate-600 hover:bg-slate-50 hover:text-slate-900'}`}>
        ...
        <div className="p-4 border-t border-slate-100 space-y-2 bg-white flex-shrink-0">
          <button onClick={logout} className="w-full flex items-center space-x-3 px-3 py-2 text-slate-600 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors text-sm font-medium">
            <LogOut size={18} />
